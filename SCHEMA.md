...
## 27. The document engine v2: clients, not vehicles; typed discounts; server-recomputed lines (migrations 061-065, Prompt 38 Phase A)

Phase 2's issuance tables (§26) stay exactly as they were - legacy, read-only in practice, still holding two real Copart test rows. A real invoice is not always about a won vehicle (INV-0025 was a repair on a car never bought at auction), so a **new, separate document engine** replaces the writer for everything going forward. Design: `docs/verification/prompt38-phaseA.md` (verifier); the resolution lives in `PLAN_TRACKER.md` §4.35.

**Numbering (061).** `document_sequences` gains `prefix` (default `INV-`/`RET-`/`CN-`/`REC-` by kind) and `pad` (minimum digits; a longer number is never truncated). `document_numbers.kind` widens to `invoice|retainer|credit_note|receipt` - the ledger, guard, void-keeps-its-number and never-reused rules from 058 are otherwise untouched. `configure_document_series(org, kind, prefix, pad, next_seq, user, reason)` can only set the counter **above** every number already in that org/kind's ledger (checked against `max(seq)`), so it can move a real org's sequence forward to continue a paper trail but can never cause a duplicate; every call is logged, append-only, in `document_series_log`. `peek_next_document_number(org, kind)` computes the next text with no lock and no side effect - safe to call any number of times.

**Org profile and tax (062).** `org_billing_profile` (one row per org: legal name, header lines, logo storage path, payment instructions, footer notes, whether per-line origin footnotes print) is superadmin-write, RLS-read for staff. `tax_codes` and `billing_defaults` are **dated** rows (Phase 1's write-guard pattern: an API role may only close a live row with `effective_to`, never edit or delete; every write logged to `rate_change_log`) - the VAT rate is never a constant in code, and the service-fee default is a data row a builder pre-fills and staff can override per invoice.

**Documents (063).** `billing_documents`: `doc_type` (`invoice|retainer|credit_note`), `invoice_kind` (`vehicle_purchase|retail|repair`, required only for invoices), `client_id` required, `won_vehicle_id` **or** `external_plate/external_vin/external_description` - never both (`billing_documents_vehicle_exclusive`). Currency and settlement currency can differ (`fx_rate`/`fx_basis` `agreed|live`/`fx_source`/`fx_rate_date`, all-or-nothing with the currency mismatch: `billing_documents_fx_shape`). Every arithmetic column (`subtotal`, `line_discount_total`, `invoice_discount_amount`, `tax_total`, `tax_breakdown` jsonb, `total`, `applied_at_issue`, `balance_at_issue`, `settlement_balance_at_issue`) is written once at issue by `issue_billing_document()` and never touched again - `billing_documents_guard` refuses any UPDATE except setting the three void columns together, and DELETE is refused outright. `billing_document_lines`: `quantity`/`rate` both `> 0` (a negative or zero line is structurally impossible - there is no way to represent a deposit as a line), a typed `discount_type`/`discount_value`/`discount_amount` (the stored `net_amount = gross_amount - discount_amount`, enforced by CHECK), `tax_code` + the `tax_rate` frozen at issue, `client_visible`, and `origin` (`computed`|`document_backed`|`staff_entered`) each with its own required companion (`source_ref`+`computed_inputs` / `source_document_id` / `basis`). `billing_document_lines_seal` (BEFORE INSERT) allows a line only in the same transaction that inserted its document (`recorded_at = now()`); `billing_document_lines_guard` refuses UPDATE and DELETE outright.

`billing_compute(p jsonb) RETURNS jsonb` is pure SQL and is the **arithmetic authority**: given lines + an invoice discount + an adjustment + the org/date (for the dated tax lookup), it returns every line's gross/discount/net, section subtotals, the tax breakdown per code, and the total. `check_billing_document()` is a **deferred constraint trigger** on both `billing_documents` and `billing_document_lines` (fixed in migration 065 to read the row id via `to_jsonb(NEW)`, since the two tables don't share a column name for it) that re-runs `billing_compute()` at commit and refuses if any stored figure disagrees - a row inserted directly with a fabricated total is refused, proved live. The same trigger enforces the kind rules (a retail invoice: exactly one client-visible line, kind `all_inclusive_price`, no hidden lines outside retail) and requires a `scope_statement` on purchase/retail invoices - replacing the old six-component "PARTIAL INVOICE" rule with one free-text field the database requires be non-blank, not a computed truth. `billing_documents_insert_check` requires the number to be an `allocated` row of the same org/kind/seq/text from the ledger, and that a linked won vehicle belongs to the chosen client. `TypeScript mirror: supabase/functions/_shared/documentMath.ts` (`computeDocument`, integer cents via BigInt, no floating point) - the Edge Function computes with this and cross-checks its own total against `billing_compute()`'s before issuing, so the two implementations cannot silently drift.

**Money (064).** `billing_payments` (client-anchored, append-only, void-with-reason; a `purpose` of `deposit`|`payment` only changes the printed label). `billing_applications` links a payment (or a paid retainer's `retainer_credit`) to a target document; `billing_applications_guard` enforces: same client on both sides; a same-currency application is 1:1, a settlement-currency payment converts at the **target document's own frozen rate** (`billing_documents.fx_rate`) with no other conversion path allowed; the sum of a payment's applications can never exceed the payment; the application can never exceed the target's outstanding balance (computed on the fly via `billing_doc_balance()`, never stored). `billing_documents_credit_note_trg` requires a credit note to name a live invoice of the same client and currency and refuses one larger than what remains outstanding (checked cumulatively - two credit notes together cannot exceed it either). `billing_documents_void_check_trg` refuses voiding a document with any live application or credit note against it - void the application/credit note first, or issue a credit note instead of voiding an invoice with money on it. `billing_document_balances` (a `security_invoker` view) is the **only** place a balance exists: `total - applied payments - applied retainer credits - live credit notes`, plus the settlement-currency figure at the frozen rate. `billing_receipts` mirrors Phase 2's receipt shape (one live receipt per payment, numbered from the shared ledger, immutable, void-with-reason).

**The print model.** `supabase/functions/_shared/documentModel.ts` (types only) is the contract between the engine and the renderer: `documentPrint.ts` turns a `PrintDoc`/`PrintDocLine`/`PrintApplication` (already-computed figures from the database, nothing recomputed) into an `InvoicePrintModel`/`ReceiptPrintModel`; `documentTemplate.ts` (written by a builder subagent, `docs/SOLVED.md` topic 49) is a pure renderer - two table densities (`showDiscount` on/off), one logo (embedded PNG, never redrawn), embedded DejaVu Sans subset fonts via fontkit (so the naira sign prints correctly; pdf-lib's standard fonts cannot), multi-page support with the table header repeated and the totals kept together on the last page. Fonts are shipped to the Edge Function as base64 in `_shared/assets/fontData.ts` (regenerate with `scripts/generateFontData.py`), not as files, since Deno bundling has no file-read access to the deployed function's own assets at runtime.

**Debt #79 (closed).** `supabase/functions/_shared/costComponents.ts` + `costReads.ts` hold the auction-fee/trucking/freight/duty logic (moved from `bidHeadroomService.ts`, database client injected via a small structural `Db` interface so the same code runs against the browser's client and the Edge Function's service-role client). `supabase/functions/billing/index.ts`'s `issue_document` mode recomputes every `computed` line from these functions at issue time and refuses the document if the figure differs from what was submitted, by even a cent (compared in integer cents, no tolerance) - proved live against the deployed function. `duty` has no calculator and `ocean_freight` has no rate loaded, so a `computed` line naming either is refused outright regardless of figure; those stay `staff_entered` (with a basis) or `document_backed` (linked to an uploaded document).

**Access.** All eleven new tables are staff SELECT-only (`org_id IN user_org_ids() OR is_superadmin()`); `supabase/functions/billing` is the only writer, staff-only (filters `memberships.role !== 'client'` - see §28/DECISIONS 19 for why that filter already existed before any client account can reach it).

## 28. Client RLS, document visibility and provisioning (migrations 066-070, Prompt 39 Phase B)

**RLS foundation (066).** `user_is_staff(p_org_id)` - a live `staff`/`superadmin` membership in THAT specific org (parameterised, not global). `current_client_id(p_org_id)` - resolves to a client record only through BOTH the identity link (`clients.user_id`) AND a live, unrevoked `client`-role membership; either missing and it returns NULL, so every client policy below returns no rows. `memberships` gained `revoked_at`/`revoked_by`/`revoke_reason` (070); `user_org_ids()`/`is_superadmin()` both exclude revoked rows.

**Staff tightening (067).** Every policy in the database whose USING/WITH CHECK was the canonical `org_id IN (SELECT user_org_ids()) OR is_superadmin()` (59 policies, found by querying `pg_policies` directly, not a hand-picked list) was rewritten to require `user_is_staff(org_id)` too. Proved to change nothing for staff (identical row counts, before and after, across five tables). Two unconditional `SELECT USING (true) TO authenticated` policies beyond the ones debt #76 already named - `assets`, `sightings` - were dropped in the same migration (any authenticated user, any org, could read either table). Direct Storage access to the `won-vehicle-documents` bucket moved to staff-only (a client with the same shape as a staff member could otherwise list/download every file in the org's folder, independent of any row-level flag).

**Client read policies (068-069), added as separate policies, never merged into the staff ones.** `clients`, `client_briefs`, `research_runs` (only rows with `client_id` set to them - "genuinely shared", not every run on their brief), `research_run_listings` (via the run's `client_id`), `won_vehicles`, `won_vehicle_status_history` (via the vehicle's `client_id`), `billing_documents`, `billing_document_lines` (`client_visible = true` AND the document resolves to them - the one place the retail-hides-cost-lines rule is enforced for a client read), `billing_payments`, `billing_applications` (no `client_id` column - matched through whichever of `document_id`/`payment_id` is populated), `billing_receipts`. Two `security_invoker` views project only client-safe columns where RLS alone cannot (row policies filter rows, not columns): `my_client_record` (never `notes`/`assigned_agent`) and `my_won_vehicles` (never `asset_id`/`research_run_listing_id`/`run_id`/`share_token`).

**Document visibility (069).** `won_vehicle_documents.client_visible` (default false) + `client_visible_by`/`client_visible_at`, settable only through `set_document_client_visibility()` (staff-only; the table still has no blanket authenticated UPDATE policy). `won_vehicle_documents_visibility_guard` refuses flagging a cost document (`document_type IN ('invoice','receipt')`) visible when the vehicle has a live `billing_documents` row with `invoice_kind='retail'`. The client SELECT policy requires both the flag and vehicle ownership.

**Provisioning (070).** `link_new_auth_user_to_client()` (the pre-existing Google-link trigger) now also inserts the `client`-role membership it always should have, on the same match it already used. `provision_client_account(client_id, user_id, actor)` is the deliberate staff path (does not rely on email-match heuristics; refuses to re-point an already-linked record to a different identity; idempotent - a second call reuses the same membership, and reactivates one that was revoked). `revoke_client_access(client_id, actor, reason)` requires a reason and immediately removes RLS-level access (proved live: reading own rows returns zero rows the instant `revoked_at` is set).

**Server-side (Edge Functions).** Every staff-only function now excludes `role='client'` from its org-access check, not just RLS (RLS is not the boundary for service-role code, which bypasses it entirely). One function - `won-vehicle-status`'s `advance` mode - had NO check of any kind until a live hostile-client test found it (debt #82, closed). `billing`'s `file_url` mode and a new `client_file_url` mode on `won-vehicle-documents` let a client fetch a short-lived signed URL for their OWN file only (`billing_files.client_id` / the document's `client_visible` flag and vehicle ownership) - the only two paths a client ever reaches a stored PDF through; direct bucket access stays staff-only.

**Client dashboard.** `src/services/clientPortalService.ts` (every read goes through the policies above, no filter of its own) and `src/components/ClientDashboard.tsx`, routed in `App.tsx`'s `AuthGate` on `role === 'client'` before `MainDashboard` is ever considered.
