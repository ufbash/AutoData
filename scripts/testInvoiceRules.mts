...
const L = (kind: any, usd: number, extra: Partial<LineInput> = {}): LineInput => ({ kind, description: kind, amount_usd: usd, origin: 'computed', source_ref: 'src-' + kind, ...extra });
...
t('computed zero (a silently zeroed abstention) -> refused', { hat: 'brokerage', currency: 'USD', lines: [L('vehicle_price', 1700), L('duty', 0)], excluded: [ex('auction_fees'), ex('inland_trucking'), ex('ocean_freight'), ex('brokerage_fee')] }, { ok: false, err: /zero amount is refused/ });
t('a STAFF-ENTERED zero for ocean freight with basis "n/a" (the verifier\'s bypass) -> refused', { hat: 'brokerage', currency: 'USD', lines: [L('vehicle_price', 1700), L('auction_fees', 640), L('inland_trucking', 475), L('ocean_freight', 0, { origin: 'staff_entered', basis: 'n/a' }), L('duty', 900), L('brokerage_fee', 500, { origin: 'staff_entered', basis: 'agreed' })], excluded: [] }, { ok: false, err: /zero amount is refused/ });
t('a BLANK amount (Number("") = 0 in the form) -> refused', { hat: 'brokerage', currency: 'USD', lines: [L('vehicle_price', 1700), L('ocean_freight', Number(''), { origin: 'staff_entered', basis: 'quote' })], excluded: [ex('auction_fees'), ex('inland_trucking'), ex('duty'), ex('brokerage_fee')] }, { ok: false, err: /zero amount is refused/ });
t('a computed line with no source -> refused', { hat: 'brokerage', currency: 'USD', lines: [L('vehicle_price', 1700, { source_ref: null })], excluded: [ex('auction_fees'), ex('inland_trucking'), ex('ocean_freight'), ex('duty'), ex('brokerage_fee')] }, { ok: false, err: /carry the source/ });
t('a real, stated zero for a WAIVED BROKERAGE FEE is allowed', { hat: 'brokerage', currency: 'USD', lines: [L('vehicle_price', 1700), L('brokerage_fee', 0, { origin: 'staff_entered', basis: 'waived by agreement' })], excluded: [ex('auction_fees'), ex('inland_trucking'), ex('ocean_freight'), ex('duty')] }, { ok: true, scope: 'partial', total: 1700 });
