...
- **Stage 1 (debt #46, PARTIALLY addressed — stays open, per Bashir 12 Sep 2026):**
  ...
  wrong for the reason above and reported rather than forced to fit. **Verification gap:** proven
  only against the database directly — the deployed HTTP path has never run it, since the shared
  secret lives only in the Chrome extension. Closing that needs a real capture session (Copart +
  bid.cars pair, and an IAAI lot logged-out then logged-in) — see debt #46's own entry.
