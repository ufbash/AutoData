...
**Status: Phase 1 built and verified; not yet committed, deployed, or cleaned up (see "Not done"). Phase 2 (the lean CRM) NOT STARTED** - the prompt's own rule 9 ("Phase 1 complete and Phase 2 not started is a perfectly good outcome") applied when the budget ran short; nothing of Phase 2 exists.
...
**Not done / needs Bashir:** (a) **Deploy** `asset-merge-candidates` (dismissal filter) and the new `asset-merge-dismiss` - until then the Dismiss/Restore buttons fail; (b) **apply 056**; (c) **push**, then cleanup migration 057 (drop `auction_fee_brackets.member_account` and `org_settings.copart_payment_tier`, add the fee-role and tier shape CHECKs) - deliberately last because the deployed app still reads those columns; (d) commit; (e) real IAAI data (debt #36) - the mechanism is proven, the data is still Bashir's; (f) decisions #69 and #70 below. **Phase 2 not started.**
