...
| 75 | The brokerage fee schedule is still provisional (found Prompt 37 Phase 2) | 7% first $20k / 5% to $60k / 3% above, $500 floor, VAT 7.5% on the fee (`DECISIONS.md` 2.5, "recommended, not yet adopted"). Invoices take the fee staff-entered with its basis; once adopted it goes in as `service_fee` rate rows (data), never as a constant, and the builder can pre-fill from it. **Needs Bashir** |
