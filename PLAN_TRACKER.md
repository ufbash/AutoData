...
- **Stage 1 (debt #46, resolved):** `research-capture/index.ts` now upgrades a VIN-less asset in
  place the moment a VIN-bearing capture of the same physical car arrives, rather than creating
  a second, permanently split asset. Upgrade only, never a downgrade-merge. The master prompt's
  own two premises about how to test this were both found wrong and reported, not silently
  worked around: the specified detection query doesn't actually catch the real known split
  (different platforms parse model/trim differently), and `assets.fingerprint_hash unique not
  null` makes "multiple ambiguous candidates" structurally unreachable. The real, reachable
  abstention (a vinless lookup finding a candidate that already carries a *different* VIN) was
  built and proven with synthetic data instead, since it has no live positives. Existing
  production splits are reported, never auto-merged.
