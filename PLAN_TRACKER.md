...
- **Stage 1 — bought-car thumbnail and detail: DONE, verified in browser.** Thumbnail in the brief from
  ...
  **Stated limits:** (1) *the final winning bid was not recorded anywhere* — the snapshot holds only
...
### 4.25 Debt #60: the real winning bid, with bid method — **DONE** (20 Sep 2026)

Migration 049 (`won_vehicle_winning_bids`), Edge Function `won-vehicle-winning-bid`, `WonVehicleWinningBid.tsx`, and an optional `bidMethod` on
`getAuctionFeeComponent`. Details in `SCHEMA.md` §22 and `DECISIONS.md` §15.9-15.11.

**What it does.** Staff record the price the lot actually hammered at (USD; capped at $1,000,000 to catch a stray digit), optionally how it was placed
(proxy or live) and an optional evidence document of the same vehicle. It sits **next to** the frozen approved price and never touches `won_snapshot`.
Append-only: a replacement needs a note and leaves the earlier entry in the history; any entry can be voided with a reason (voiding a replacement makes
the earlier entry current again); no edit, no delete, even for the service role. The bought-car view prices auction fees **at the recorded winning bid**
(labelled so) and, when the method is known, the bid fee is **exact instead of a range**; with no bid recorded it is unchanged (approved price, range).

Verified against the deployed endpoint with two synthetic won vehicles (all soft-deleted, the bids voided and retained; the real Yaris untouched):
- Fee vs independent SQL, all four cases exact: $1,700 proxy **$797.50**, live **$807.50**; $5,000 proxy **$1,265.00**, live **$1,280.00**; and $1,650 live **$780.00**
  (buyer fee $555 from SQL). **The no-method baseline is unchanged** ($802.50 / $1,272.50), so the ListingCostBreakdown/headroom callers are unaffected.
- Rejected: zero, negative, non-numeric, over-cap, bad method, unknown vehicle, soft-deleted vehicle, evidence from another vehicle, evidence that does not
  exist, replacing without a note, voiding without a reason, voiding twice.
- Database guards (uncommitted transaction, nothing persisted): edit amount, edit method, hard delete (as service role), rewrite a voided row, blank void
  reason, the amount/method CHECKs, wrong org for the vehicle, evidence from another vehicle (composite FK) all refused.
- `won_snapshot` byte-identical after record, replace and void. The other vehicle saw none of the first vehicle's bids. Logged out: `[]` on read, RLS denial on
  insert, 401 on the function.
- UI: the panel shows amount, method, evidence and the difference from the approved price; Save is disabled until a replacement note is entered; Costs re-priced live.
...
### 4.27 Debt #61: the bid.cars auction-house mislabel — **DONE** (20 Sep 2026), ingest path not yet exercised live

`_shared/bidcarsLot.ts` (the one mapping), `research-capture` (derives it at ingest), `sightings-platform-relabel` (re-runnable backfill), `content-bidcars.js` (stops classifying).
No migration. Details in `SCHEMA.md` §24, `DECISIONS.md` §16, `docs/SOLVED.md` 38.

**Root cause.** The extension derived `source_auction_platform` from the numeric prefix of bid.cars' "Lot" field: `0` or `1` → copart, `2` → iaai. Measured on all 169 real
bid.cars captures, that had `0` backwards: prefix `0` has 18 cities only IAAI has yards in and none only Copart has; prefix `1` is the reverse (49 Copart-only, 0 IAAI-only); and bid.cars' own
sales-history table says IAAI on all 41 prefix-`0` assets and Copart on the 71 prefix-`1` assets. Prefix `2` has never been observed, so its old "→ iaai" was unevidenced.

**What was wrong in production.** 50 IAAI lots labelled Copart and 12 unlabelled: their fee panels priced an IAAI car under Copart's schedule, and 24 "matched" a Copart yard and quoted Copart
trucking. 40 live run listings across 15 runs; the one approved listing and the one won vehicle were the Yaris. Nothing is stored that derives from the label (fees are computed on read), and the
frozen approved/won snapshots hold no platform label, so nothing needed recomputing.

**The fix.** Mapping now only what the data proves (`1` copart, `0` iaai; anything else null → downstream abstains). `research-capture` derives from the raw snapshot and **ignores whatever the
extension sent** (a stale extension in a browser cannot reintroduce it); the value the extension sent stays in `raw_payload` (§5.8) beside a `source_auction_platform_derivation` stamp. The backfill uses the
same module (no second copy of the mapping in SQL), dry-run by default, changes a row only when the derived house is known and differs, and stamps `raw_payload.platform_relabel {from, to, basis, at, by}`.

Verified: the TS parser agrees with the SQL evidence on all 169 snapshots; the deployed dry run reported exactly the independently-measured 62 (50 copart→iaai, 12 null→iaai; 100 unchanged, 7 unresolved
for no Lot line) and wrote nothing; apply changed 62; a rerun changes 0 (idempotent); the Yaris row is `iaai` with a stamp `from: copart` and the raw extension value still `copart`. Effect on the real
matcher: 12 unlabelled lots and 17 mislabelled ones gained a correct yard match, 24 moved from a wrong Copart yard to the right IAAI one, 9 stay unmatched. Regression: correct Copart lots still price
($1,065 at a $3,000 bid); relabelled IAAI lots abstain with "no stored fee schedule for platform iaai", and the Yaris's fee note now says that instead of the contradiction message.

**Visible consequence.** For those lots the fee panel goes from a (wrong) Copart-priced number to "not quotable". That is correct — IAAI has no stored fee schedule (an IAAI invoice is needed) — but it looks like a regression.

**Outstanding, needs Bashir's browser:** the *ingest* path is not exercised live (it needs the extension and the capture secret, neither available to a coding session). Reload the extension at `chrome://extensions`
**and** hard-refresh, capture one bid.cars lot whose Lot reads `0-…`, and confirm the new sighting has `source_auction_platform = 'iaai'` and a `source_auction_platform_derivation` stamp in `raw_payload`; capture one `1-…` lot and confirm `copart`.

### 4.28 Debt #66 (part): the four unambiguous port typos — **DONE** (20 Sep 2026)

`scripts/lib/truckingRatesParser.mjs` (`PORT_ALIASES`, the importer's own reviewed table), `scripts/relabelTruckingPorts.mjs` (generates the correction SQL from that same table). No migration.

**What changed.** `BATIMORE` → `BALTIMORE` (2 Copart rates: North Charleston SC, Spartanburg SC), `PROVDIENCE` → `PROVIDENCE` (3 IAAI: Boston-Shirley, Taunton, Templeton MA), `WILLMINGTON` → `WILMINGTON`
(1 Copart: Seaford DE), `MD-Baltimore` (raw) → `BALTIMORE` (1 Copart: Houston TX). 7 rows. The importer's rule is "never silently fix a name that isn't in this table", so these were reviewed one by one first: none of the
7 rates had a correct-port rate at the same yard (no duplicate created), each yard's geography fits its target, and no saved destination referenced a typo.

**Reclassification, not a new rate.** Only the derived `destination_port_normalized` moved; `destination_port_raw` (what the vendor wrote), price, yard, vendor and dates are untouched (§5.8). The generator reads
`PORT_ALIASES` rather than restating the mapping in SQL, so there is one definition and a future re-import applies it. Idempotent: a rerun matched nothing.

Verified against the database: Baltimore/container 251 → **254**, Providence/roro 24 → **27**, Wilmington/roro 8 → **9**, exactly as predicted; the raw strings are preserved on all 7 rows; the four corrected yards now
resolve through the same lookup the cost view uses (Spartanburg→Baltimore container $550, Houston→Baltimore container $775, Taunton→Providence roro $325, Seaford→Wilmington roro $350); `destination_options()` no
longer lists any typo variant (the remaining 1-rate entries are `DAVISVILLE`, `GA-RINCON` and `TEXAS`/roro, all legitimate or deliberately left).

**Left alone.** See debt #66: `MIAMI PORT`, `GA-RINCON`, `DAVISVILLE`, the regional labels. The remaining debt #67 items (leading vendor code on `IAA Dallas/Ft Worth`, `Minneapolis South` — the vendor's IAAI sheet has only
`Minneapolis/St. Paul` and `RICE` in Minnesota, so nothing in the data links them) are also a human's call.

### 4.29 Verification and data entry after the auction-house fix, and the first IAAI purchase invoice — 20 Sep 2026

**Debt #61's ingest path is verified live** (see the debt row). **Debt #65 (tier 2) is resolved** by Bashir's decision to hide lists A and B (133 makes).

**The real Yaris now carries real figures** (recorded through the production endpoints on Bashir's instruction): winning bid **$1,700.00** (evidence: the IAAI buyer invoice, filed as a `receipt` document, not an `invoice` document,
because the Invoice section lists `invoice`-type documents as candidates to issue to a client and this is IAA's bill to Caplimo; the invoice total is in the bid's note), and destination **FREEPORT (roro)**. The bid method (proxy/live)
is not on the invoice, so it is not recorded and the bid fee stays a range in the view. The fee still abstains (no IAAI schedule), trucking abstains (the yard is unmatched, debt #67) and shipping abstains (no ocean-freight rate, debt #35).

**The IAAI purchase invoice** (`Invoices/5de199d9-….pdf`, "Buyer Invoice Receipt", Insurance Auto Auctions): buyer **#697168 EROLDS AUTO LIMITED** (Ebute-Metta, NG — an intermediary buyer account, like White Nexus for Copart, not Caplimo's
own name), stock **45905795**, VIN `JTDBT923781219099`, 2008 Toyota Yaris, branch **Dallas/Ft Worth**, won 14 Sep 2026, payment due 18 Sep. **Bid $1,700.00 + fees $645.00 + IAA transport $0.00 + tax $0.00 = $2,345.00.** The invoice states fees only
as one total, with no breakdown. Its Branch column is the first primary-source confirmation that bid.cars' `IAA Dallas/Ft Worth (TX)` is the IAAI branch `Dallas/Ft Worth` (the yard-matcher's leading-"IAA " gap, debt #67).

**Is $645 accurate against IAAI's published fees? Partly verifiable, honestly:**
- IAA's own tables are published as images behind bot protection (Incapsula), so the official brackets could not be read directly. The per-unit fees are confirmed on IAA's own pages (effective 1 Jul 2026): service **$105**, environmental **$15**,
  title-handling **$20**. The bracket tables were read from a broker's reproduction of IAA's *Standard* schedule (SCA Help Center, "last updated Jul 01, 2026"): buyer fee for a **$1,700.00-$1,799.99** sale is **$485.00**; internet bid fee for
  **$1,500-$1,999.99** is **$85 proxy / $95 live online**.
- **Standard-schedule expectation at $1,700: $485 + $85 + $105 + $15 + $20 = $710 (proxy), $720 (live). The invoice is $65 / $75 LOWER.** So it is not an overcharge against the Standard schedule.
- The gap is consistent with IAA's *High-Volume* licensed schedule (lower buyer fees; it applies at 25+ units and $75,000+ in 12 months on fewer than 5 bidder accounts): $645 - $140 flat fees - $85 proxy = **$420** implied buyer fee. That is an
  inference from arithmetic, **not a confirmed fact** - the High-Volume bracket table could not be read from any source I trust, and the invoice does not itemise.
- Two other sources were discarded as unreliable: a UK (GBP) IAA schedule, and an older US article using stale flat fees ($79 service, $10 environmental).

**To close it (needs Bashir):** the itemised fee lines for stock 45905795 from the IAA account (the buyer portal's invoice detail lists each fee), and/or IAA's High-Volume fee PDF downloaded from a logged-in session. Upload either through Document Extraction:
the schedule as `official_tariff`, this purchase as `actual_paid`. For pricing IAAI lots the code also needs a small follow-up (`getAuctionFeeComponent`, the max-bid solver, the hard-coded Copart flat-fee labels and the default member account are Copart-only);
`auction_fee_brackets` already accepts `iaai`. A suitable member account name would be `EROLDS AUTO LIMITED (IAAI #697168)`.

### 4.30 The IAA-prefix fix, and the 1,000-row cap it uncovered — 20 Sep 2026

`yardMatchingService.ts` (leading "IAA " stripped for the IAAI network only), `truckingRatesService.ts` (`listActiveYardKeys`, paged `searchTruckingYards`), and the three call sites. No migration, no deploy beyond the frontend.

**Debt #67 (leading "IAA ").** IAA's own buyer invoice for the Yaris names the branch "Dallas/Ft Worth"; bid.cars wrote it `IAA Dallas/Ft Worth (TX)`; the vendor sheet has the yard as `Dallas/Ft Worth`. Stripped for `effectivePlatform === 'iaai'` only. Measured over all 204 real sightings against
the full yard list: exactly 1 sighting changes (`unmatched → matched`, the Yaris), all 179 existing matches keep the identical yard, no IAAI yard begins with "IAA".

**The catch.** The fix worked in a Node test against the complete yard list but the app still said "no iaai yard found for DALLAS/FT WORTH, TEXAS" - the prefix was stripped, the yard simply was not in the list the app had loaded. The cause was debt #68: the cost views loaded the yard list with a bare select and got
...
| 63 | **Open** | Re-counted: **8 inline copies** still present (`ClientsList.tsx` ×4 incl. one differently spaced, `ResearchRuns.tsx` ×2, `ResearchRunDetail.tsx` ×1, `researchService.ts` ×1); only the run card uses the shared `briefReference()` | Switch the 8 to the shared function. **Claude Code** - small, same shape as #47 / #58 **RESOLVED 21 Sep 2026 (Phase 1, first item):** the **7 copies that produced exactly the shared output** now call `briefReference()` - `ResearchRuns.tsx` ×2, `ResearchRunDetail.tsx` ×1, `ClientsList.tsx` ×3, `researchService.ts` ×1 (the deposit-check message). Proven identical to the old inline expression over all 900 combinations of null / undefined / 0 / real values, and the Clients, Research Runs and run-detail screens render the same references with no console errors. **The eighth copy was unified the same day on Bashir's confirmation:** the recycle-bin list of deleted briefs in `ClientsList.tsx` wrote `make||'Any'` / `model||'Any'`, so an empty make and model read "Any Any"; it now reads "Any Make Any Model" like every other screen (the one deliberate visible change, and only for briefs with no make or model). All eight inline copies are gone; `grep "'Any Make'"` in `src/` finds only the shared function. The year-only field renderers (`ClientsList.tsx`, `ResearchRunDetail.tsx`) and the intake summary are different strings, not copies, and are untouched |
...
| 66 | **Partly closed** -> **Stage 4 below closes `MIAMI PORT`** | Four typos aliased (§4.28) | After Stage 4: only the deliberately left-alone labels remain, each with its reason recorded |
| 67 | **Partly closed** -> **Stage 4 records the rest** | Hyphen fold and leading "IAA " strip shipped; the matcher is otherwise unchanged | After Stage 4: procurement items (Bashir -> vendor) and the capture-side `Dream Rides ()` issue |
| 68 | **Closed** | Shared helper, completeness check, all 531 yards visible. *Correction note added:* its claim that only one other table exceeds 1,000 rows is superseded by the Stage 2 audit | - |
...
### 4.33 Prompt 37: Stage 0 and Phase 1 - admin consolidation and the rates architecture (21 Sep 2026)

**Status: Phase 1 built, verified, migrated (051-056 applied), Edge Functions deployed and committed (4 commits, not pushed); cleanup migration 057 applied; only the real IAAI data remains (see "Not done"). Phase 2 (the lean CRM) NOT STARTED** - the prompt's own rule 9 ("Phase 1 complete and Phase 2 not started is a perfectly good outcome") applied when the budget ran short; nothing of Phase 2 exists.

#### Stage 0 - the unattributed write

The confirmed `UPDATE trucking_rates ... 'MIAMI'` in Prompt 36 matched 0 rows because the 12 rows were already `MIAMI`. **Cause not determinable; what was established:**
- It ran **once, as one statement, as the database admin role (`postgres`)** - a direct SQL session (CLI, dashboard editor or `psql`), not the app, an Edge Function, a database function or a trigger (`pg_stat_statements`: role `postgres`, +1 call, +12 rows against the 7 earlier typo corrections; no trigger or event trigger touches the table).
- It landed in the window when the assistant was idle awaiting confirmation (about 23:58-00:53 UTC): the assistant's own command log shows no `UPDATE` there, and no other Claude session was running. The only cron job is the monthly backup.
- Most probably Bashir ran the exact statement shown in the confirmation request. Not provable: the database keeps no per-statement timestamp or user.
- **A finding in its own right, closed by migration 051:** the three rate tables let **any org member** UPDATE or DELETE any row through the API (permissive RLS), had no triggers and no `updated_at`, so no write could be attributed afterwards - while `PROJECT_CHARTER` 5.10 ("never edited in place") lived only in application code. Now API roles may only *close* a live row (set `effective_to` once); no other edit, no delete, no TRUNCATE; INSERT/UPDATE policies are superadmin-only; and **every** write, including from the admin role, is recorded in `rate_change_log` (who, role, `auth.uid()`, application, old and new row). Proven in a transaction that always aborted: edit price - refused; edit port - refused; truncate - refused; close before start - refused; close properly - allowed once; close again - refused; an API delete removes 0 rows; the log recorded `INSERT/postgres` and `UPDATE/authenticated`. The next unattributed write will be attributable.

#### Phase 1 - what was built

**The goal, as a test: adding IAAI's fee schedule is a data operation, not a code change.** It is now true; the proof is below.

| Piece | What |
|---|---|
| Auction house is a key | `auction_houses` (copart, iaai, manheim, adesa) with `location_prefixes` replacing the hardcoded IAA-yard regex. The `auction_platform` CHECK lists on the fee and trucking tables became foreign keys |
| Official tiers, not people | `auction_fee_tiers`, named as the house publishes them: `Copart U.S. Non-Licensed`, `Copart U.S. Licensed - Low Volume`, `Copart U.S. Licensed - High Volume` (Copart's own U.S. fee pages, read 21 Sep 2026; Copart labels the two Licensed paths by criteria, not by name, so "Low/High Volume" is our shorthand and the verbatim criteria are stored); IAAI: `IAA U.S. Public`, `IAA U.S. Non-Licensed`, `IAA U.S. Licensed` (named from IAA's document titles, contents unreadable). The 216 and 108 existing rows moved from a person's/company's name to `Copart U.S. Non-Licensed` and `Copart U.S. Licensed - High Volume`; the holders are now plain facts on the account |
| Accounts and payment tiers per house | `auction_accounts` (tier, holder, member number, Secured/Unsecured **per account**, one default per house). `org_settings.copart_payment_tier` and `DEFAULT_MEMBER_ACCOUNT` no longer exist in code |
| Flat fees by role, not label | `cost_rates` gained `auction_platform`, `fee_role`, `fee_applies` (always / contingent); the code sums a house's `always` fees and names none. Amounts, dates, sources untouched |
| One pure fee core | `supabase/functions/_shared/feeSchedule.ts` (no imports), plus `rateConventions.ts` (dated rows, source, org, currency via `usdAmount`). `bidHeadroomService.ts` now only resolves *which rows* and calls it |
| Abstention survives, and is sharper | Every path returns `unavailable` with a reason (no platform / unknown house / no account / tier with no rows / ambiguous schedule / unclassifiable title / no price / no rows for this title-payment / no bracket). A percent bid fee makes the max-bid solver abstain rather than solve wrongly. A **partial** fee (bid fee missing) now also blocks a headroom figure (it did not before) |
| Conventions unified across the three tables | Guard + audit (051); live-row uniqueness on `cost_rates` (per house and role) and `auction_fee_brackets` (per schedule cell); one currency reader; the supersede form no longer drops currency (a non-USD rate is replaced via Document Extraction where the FX is frozen). **Trucking has no unique index yet**: two live duplicate groups exist (debt #69) |
| Admin | One **Admin** entry (superadmin) replaced four tabs; sections **Rates** (Cost Rates, Trucking Rates, Fee schedules and accounts) and **Review queues** (Document Extraction, Asset Merges) on a shared `ReviewQueueShell`. Asset Merges gained the missing negative decision (`asset_merge_decisions`, append-only, voidable) with Pending / Dismissed / Merged tabs; Document Extraction's silent refresh no longer unmounts an open review |
| A loader for data | `scripts/loadFeeSchedule.mjs <json> --org <uuid> [--remove]` prints SQL (never runs it) and validates shape |

**Migrations 051-055, each applied and checked before the next** (051 guard + audit; 052 houses/tiers/accounts; 053 `fee_tier` on brackets; 054 fee roles on `cost_rates`; 055 merge decisions). 053 and 054 were checked with a fingerprint of every amount, date and source before and after: identical (`1631b790...` for the 324 brackets, `47c12b4f...` for `cost_rates`). Migration 056 (audit of account changes) was applied afterwards (21 Sep 2026): a tier change is logged as `UPDATE/authenticated unsecured->secured`, proven in an aborted transaction.

**Regression baseline - every Copart figure reproduces.** A frozen copy of the pre-Phase-1 arithmetic (`scripts/feeRegression/oldCore.mts`, verified line-for-line against `git show 478e8f2`) and a snapshot of the 324 real rows are diffed against the refactored core: **8 schedules x 181 prices (every bracket boundary and +-0.01) x 3 bid methods, plus bracket boundaries and 243 solver targets = 7,736 points, 0 differences.** The seven named figures reproduce exactly: $802.50, $1,272.50, $797.50, $807.50, $1,265.00, $1,280.00, $780.00 (also the clean-title $640 and $1,130, still flagged PARTIAL). The live migrated rows equal the snapshot row for row (only the tier key renamed), and the refactored service returns the same figures against the live database. **A finding, not a failure:** the *old* code's answer at a price sitting exactly on a shared bracket boundary depended on physical row order - 16 boundary pairs exist, all on the historical High-Volume account; one differs (live bid fee at exactly $8,000, $160 vs $145) because a replaced `actual_paid` row had moved to the end of the table. 20 grid points, none on the default account. The new rule is explicit: the lower bracket wins at a shared boundary (agrees with the old code on 15 of 16).

**The central proof - IAAI from data, zero TypeScript change** (run twice; the second on the *final* code after the independent verifier noted the first run pre-dated two small edits):
1. Before: the real Toyota Yaris (IAAI, IAA Dallas/Ft Worth) abstained - `no IAA (IAAI) account is set up`; 66 of 66 IAAI lots abstained the same way.
2. A synthetic, tagged schedule (`scripts/feeRegression/synthetic-iaai.json`; tier and account both named `SYNTHETIC ... (delete me)`) was loaded as generated SQL. The Yaris priced **$488** (proxy $483, live $493; $6,000 = $815.50 through the percent bracket; the contingent $999 late fee correctly not added), matching an **independent SQL computation** exactly; shown too in the real won-vehicle cost panel, the headroom flow and the bracket-boundary view.
3. Removed by the loader's own `--remove` SQL: 0 synthetic rows remain; brackets back to 324, cost rates back to 6; the audit log holds the 11 inserts and the 11 deletes. **Every IAAI lot abstained again with the stated reason; Copart still $802.50.**
4. **TypeScript diff between load and removal: empty.** A SHA-1 of every `.ts/.tsx/.mts/.mjs` file under `src/`, `supabase/functions/` and `scripts/` was identical before and after (final run: `bfaf8fa9b42d...` at 13:20:35 and 13:20:58 UTC), and `diff` of the full `git diff HEAD` + untracked-file snapshots is empty.

**No hardcoded auction-house label remains in the fee path** (grep of `bidHeadroomService.ts`, `feeSchedule.ts`, `rateConventions.ts`, `auctionAccountsService.ts` for copart / iaai / jamilu / white nexus / licensed: no hits except comments; `resolveEffectivePlatform` is now data-driven and was checked identical to the old rule across all 221 real sightings). **Adjacent, deliberately not changed:** `yardMatchingService.ts` still hardcodes copart/iaai and has its own resolver (debt #71).

**Independent verification.** A fresh subagent that wrote none of it verified six requirements cold; full report in `docs/verification/prompt37-phase1.md`. Verdicts: 1 regression figures **PASS**; 2 no hardcoded label **PASS** (yard matcher out of scope, noted); 3 synthetic proof **partly verified** - the arithmetic, loader SQL, audit-log evidence of insert-then-delete and the clean database all checked, but it found two source files modified *after* the first removal, so the final code had not been run against IAAI data (**resolved: re-proved above on the final code**); 4 abstention **PASS**; 5 admin area **PASS**; 6 nothing altered **PASS** (from the database's own audit log: 324 of 324 bracket updates and 5 of 5 cost-rate updates differ only in the new columns). Its other findings and what was done: a chosen account from a different house abstained by accident with a misleading reason - **fixed**; `fetchTierRows` unpaginated - **fixed**; trucking/ocean readers ignored the currency convention - **fixed**; second live fee failed with a raw unique violation - **fixed** (plain message); a partial fee fed headroom as complete - **fixed**; leftover UI text - **fixed**; account payment-tier changes leave no history - **migration 056 written, awaiting confirmation**; `setDefaultAccount` is two non-atomic updates - **recorded** (a failure between them leaves no default, which abstains loudly); extraction prompt still says `member_account` - **recorded** (#72). There was no material disagreement with my own assessment.

**Done after the verifier's report (21 Sep 2026):** migration 056 applied; `asset-merge-candidates` (v6) and the new `asset-merge-dismiss` (v1) deployed. The dismissal flow was tested end to end on two soft-deleted dummy assets: dismiss - ok; the same pair reversed - idempotent; restore without a reason - refused; restore - ok once, then refused; the voided ledger row is kept. **Not exercised live:** that a dismissed pair is *hidden from the pending list* - no real merge candidate exists (the scan returns none), so the filter runs on every scan but has had nothing to filter. **Not done / needs Bashir:** (a) ~~push, then cleanup migration 057~~ **done** (pushed; 057 applied - see debt #73); (b) real IAAI data (debt #36) - the mechanism is proven, the data is still Bashir's; (c) decisions #69 (his answer is recorded; it names an invoice the system cannot find) and #70 ("not sure", left as is). **Phase 2 not started.**

### 4.34 Prompt 37 Phase 2: the lean CRM - numbering, invoices, payments, receipts, the client view (21 Sep 2026)

**Status: built, tested against the live database and through the deployed function; independently verified (`docs/verification/prompt37-phase2.md`, verbatim); the verifier's material finding (a $0 or unsourced line accepted in a "complete" invoice) and its smaller ones are FIXED - migration 060 applied 21 Sep 2026, `won-vehicle-billing` redeployed (v2), refusals re-proved live (below).** Most of the CRM already existed from Prompt 34 (clients, briefs, won vehicles, documents, issuances, tracking, mailer); this phase is consolidation and the gaps, on the existing machinery. Migrations 058-059 applied; `won-vehicle-billing` deployed. Decisions: `DECISIONS.md` 18. Schema: `SCHEMA.md` 26.

**Pre-flight (three read-only subagents in parallel; only one had database access, run one query at a time - the Phase 1 lesson).** What they established: **no numbering existed** (`invoice_number` was nullable free text, no uniqueness); **no PDF library** existed; the issuance record held a single staff-typed amount (no lines, no payments, no receipts); the won-vehicle landed-cost refusal **omits the winning bid and the service fee** from its component list, so an invoice must not copy it; the brokerage fee is only **provisional** (7/5/3%, $500 floor, VAT 7.5% on the fee); the real data is small (8 live clients, 1 live won vehicle - the Yaris, 0 emails for live clients); and `research_runs` has a permissive `USING (true)` policy so RLS alone cannot isolate runs.

**Built.**

| Piece | What |
|---|---|
| Numbering | Per-org `INV-` / `REC-` sequences in a ledger (`document_numbers`) with database uniqueness; allocation locks the org's counter row; a voided number is kept and never reused; gaps are always a status |
| Generated invoices | Extend the existing issuance record; line items; hat, scope, excluded components, frozen FX; the database refuses an invoice silent about a cost component; header = sum of visible lines |
| PDF | `_shared/documentPdf.ts` (pdf-lib, `npm:` in the function) - prints hat, scope, a red **PARTIAL INVOICE** banner with every exclusion, and the frozen rate for NGN; stored in the won-vehicle document store and linked to the issuance |
| Payments / receipts | Append-only, void with a reason; balance derived (a view); overpay, wrong currency, voided invoice refused; one live receipt per payment |
| One Edge Function | `won-vehicle-billing`: `issue_invoice`, `record_payment`, `void_payment`, `issue_receipt`, `void_receipt`; idempotent; a failure after a number is allocated marks it `abandoned` and retires the document |
| Builder UI | In the won-vehicle popup: pre-fills only real components; everything else needs a real figure with its basis or an exclusion with a reason; a live summary states partial/complete before issue |
| Client relationship view | `ClientRelationship` (staff-only): briefs, all runs (including the 19 with no brief), won vehicles, documents, invoices with derived balance, payments, receipts, email history (masked recipients); every query double-scoped (org and client) and asserted after load |

**Evidence** (all against the live database, on a **synthetic second org** so the real org's `INV-000001` was not burned; it stays as a labelled `ZZ SYNTHETIC ORG` because append-only ledgers cannot be deleted):
- *Invoice rules, 13 cases in Node* (`scripts/testInvoiceRules.mts`): the Yaris shape passes as **partial $2,175** with four stated exclusions; **the same lines with shipping and duty simply left out are refused**; a computed zero, a hidden brokerage line, a staff figure with no basis, a retail invoice that itemises - all refused.
- *Database shape rules, 10 cases* (bypassing the function): silent about a component - refused; `complete` claimed with two lines - refused; hidden line, itemised retail, component both line and excluded, no basis, exclusion without reason - refused; the three valid shapes (Yaris partial, retail, complete brokerage) issued.
- *Numbering race, proved not asserted:* two sessions at once, the first holding its transaction 4 s - the second waited 4.7 s and got `INV-000002`, distinct and consecutive. *Double-issue race:* same idempotency key from two sessions - the first issued, **the second refused by the unique index**, one invoice exists.
- *Void keeps its number:* `INV-000010` voided -> ledger `voided`, the next allocation was `INV-000013`. Edit an issued invoice, un-void, delete an invoice or a line, rewrite or release a number, move a number backwards - **all refused, even as the admin role**.
- *Payments:* balance derived (5,715 invoice, 3,000 paid, 2,715 outstanding); overpay refused; wrong currency refused; **two concurrent payments of 2,000 against 2,715 outstanding - exactly one accepted, balance never over-paid**; an invoice with live payments cannot be voided; a voided payment restores the balance; edit or delete a payment refused.
- *Receipts:* `REC-000001` issued; a second live receipt for the payment refused; a payment with a live receipt cannot be voided until the receipt is; a receipt for a voided payment refused.
- *NGN frozen:* rate 1453.12345678 - each line `round(usd x rate, 2)`, total the sum, USD equivalent 2,175.50, **identical on two reads**; changing the rate afterwards refused; a forged NGN line that does not match the frozen rate refused at commit. Through the deployed function: a **live rate was fetched once** (1332.834401, source recorded); a retry with the same key returned the same invoice.
- *Rendered PDFs, read back from storage:* the brokerage invoice itemises and prints "PARTIAL INVOICE - the total above is NOT the full cost of this vehicle" with all four exclusions; the **retail invoice shows only USD 4,200.00 and no cost line**. The NGN invoice's rate text was checked in the local renderer, not read back from the stored file.
- *Auth gating:* the function with no header - 401; with the anon key - 401; the stored PDF by public URL, by direct object URL and by signing, anon key only - refused; the four new tables, the balance view and `allocate_document_number` as anon - permission denied.
- *The real Yaris in the real UI:* the builder pre-filled **only** the vehicle price ($1,700) and inland transport ($475); auction fees ("no IAA (IAAI) account is set up"), ocean freight, duty ("C2 duty calculator not yet built") and the brokerage fee were **not** lines and each carried its own abstention text as the exclusion reason; the summary read "PARTIAL invoice: total of the items listed USD 2,175.00". **Not issued** - it would have consumed the real org's first number.
- *Client relationship view:* the real client shows 3 briefs, 4 runs, 1 vehicle, 2 documents (matches the pre-flight counts) and nothing synthetic; synthetic client A shows its 8 invoices, 3 payments, 2 receipts (equal to a direct ground-truth count) and **none of client B's rows**; email history links through both briefs and vehicles and each client sees only its own; a real org with a synthetic client, and the reverse, are **refused**.

**Not verified / not done.** The UI *click* path of issuing an invoice, recording a payment and issuing a receipt was not driven in the browser (it would need the real org); the same service calls were exercised end to end. No invoice is emailed by the system (the recipient is a typed label). A deposit or commitment fee is not credited against an invoice - the documents do not say how (debt #74). The brokerage fee is staff-entered (debt #75). New debts #74-#78 below.


**Independent verification (fresh subagent, read-only, one query at a time, aborted tests only, synthetic org) - report verbatim in `docs/verification/prompt37-phase2.md`.** Eight requirements checked: numbering, pre-fill, two hats, frozen currency, payments, receipts, client view, auth gating all PASS on the core claim; the verifier said plainly that the failure I named (an invoice that totals while a component abstains) was **only partly ruled out**: silence about a component is refused, but a *zeroed* or *unsourced* line for a missing component was accepted by the database (the "computed zero refused" rule lived only in `deriveInvoice`), a blank amount field became $0 in the form, and `origin` is self-declared. I treated that as material (stop rule) and fixed it before commit: zero amounts refused on every line except a waived brokerage fee; computed lines must carry a source; a computed vehicle price is checked server-side against the winning-bid ledger; blank amount = not a number. **What is still trusted, not verified:** the other computed lines (fees, trucking, freight) come from browser cost services and cannot be recomputed on the server; they carry their source rows but a caller with a session can label anything "computed" (debt #79). Fifteen further findings (lines appendable after issue, non-ledger numbers, receipt org gap, soft-deletable PDFs, idempotency without payload compare, racing receipts, void race, TRUNCATE, PDF wording, retail internal lines, relationship-view coverage) each have a row in the resolution table in that file. Twelve ledger numbers left `allocated` are my own test allocations (invoice 2-9, 13, 15; receipts 2, 3); they were marked `abandoned` with a note on 21 Sep 2026 (confirmed); the synthetic ledger now holds 12 abandoned, 9 issued, 2 voided and no `allocated` stragglers.


**Re-test after the fixes, live (21 Sep 2026; aborted blocks in the synthetic org, so nothing persisted).** *Database:* a complete invoice with a staff-entered $0 ocean-freight line (basis "n/a") - **refused**; the same with a computed $0 - refused; a $0 vehicle price - refused; a computed line with no source or a blank source - refused; the three controls (Yaris shape with sources, a waived $0 brokerage fee with its basis, six real lines) - accepted. A generated invoice with a number not from the ledger (INV-000500) - refused; a line inserted onto an issued invoice - refused (sealed); soft-deleting a document a live invoice points at - refused; TRUNCATE of payments, receipts, lines, issuances and document_sequences - each refused by the new guard; a receipt with a random org - refused. *Through the deployed function (v2):* first issue INV-000019; same key and content - returns the same invoice; **same key, different content - 409**; a computed vehicle price with no matching winning bid - 400; a $0 line - 400; the refused calls burned no number. **Still not tested:** the racing-receipt and void-race fixes (reasoned, not raced), the PDF text after the wording change (not re-read from storage), and the UI path in a browser.


### 4.35 Prompt 38 Stage 0 and Phase A: the document engine, server-side recomputation (22 Sep 2026)

**Status: built, migrated (061-065 applied), deployed (`billing` v1), tested against the live database through the deployed function; independently verified (`docs/verification/prompt38-phaseA.md`, verbatim) - no material finding; the verifier reconstructed the double-subtraction and tampered-line failures itself and both held.**

**Stage 0.**
- **0a Miami:** Bashir confirmed he applied it himself. Closed as resolved.
- **0b Gate fee (#70):** the $95 row `18a66c6f-...` superseded (`effective_to` 2026-09-21) by a new row `Copart Gate Fee` (label only, amount, role, platform, tier all unchanged), effective 2026-09-22. Re-fingerprint of every live `auction_fee` row (platform, role, applies, value, currency, category) before and after: **identical**, and `always`-fee sum for Copart is still $130. The frozen 7,736-point regression still shows 0 differences.
- **0c Service fee (#75):** superseded by `billing_defaults` (migration 062): flat $700 + VAT (dated tax code `VAT`, 7.5%, effective 2020-02-01), editable per invoice. The INV-0027 inconsistency ($300, no VAT) is unresolved by design - Bashir confirmed 21 Sep 2026 the $700 fee **does** carry VAT; the real invoice's $300 predates this policy and is left as printed history, not restated.

**What Phase 2 got wrong, corrected here (Bashir's design, approved before any migration):**
1. **Invoices anchored to won vehicles** → a new `billing_documents` table: `client_id` required, `won_vehicle_id` **or** external plate/VIN/description, never both (`billing_documents_vehicle_exclusive`). The Phase 2 tables (`won_vehicle_invoice_issuances` etc.) are left exactly as they were - legacy, still readable, no longer the writer for new documents.
2. **Numbering starting at `INV-000001`** → migration 061 adds per-kind `prefix`/`pad` and `configure_document_series()` (can only move a counter **forward**, past every number already in the ledger - a duplicate is structurally impossible). Caplimo's real counter is now `last_seq=27, pad=4`; the next real invoice is **INV-0028**. `peek_next_document_number()` shows it without allocating (called twice in a row: same answer both times).
3. **FX fetched live with no fallback** → `fx_basis` (`agreed`|`live`), `fx_source`, `fx_rate_date` all stored; an agreed rate needs who agreed it. Still frozen at issue; `billing_documents_fx_shape` enforces the pairing.
4. **The six-component exclusion rule** → replaced by a required `scope_statement` on purchase/retail invoices (`check_billing_document`); no "PARTIAL INVOICE" banner exists anywhere in the new renderer.
5. **Deposits as negative lines** → `billing_payments` + `billing_applications` (a payment or a paid retainer's credit applied to a document); a line's `rate` must be `> 0` (`billing_lines_discount_shape`-adjacent CHECK), so a deposit cannot be represented as a line at all.

**The model.** Line items: section, quantity, rate, a **typed** discount (`none`|`percent`|`fixed`, amount stored beside it), tax code, client-visibility, and one of three **origins**: `computed` (server-recomputed, refused on any disagreement - closes debt #79), `document_backed` (must point at an uploaded document of the same org/vehicle), `staff_entered` (needs a stated basis). Discounts reduce the taxable base; the printed "Discount Applied" figure is **derived at render time**, never a stored value a second deduction could touch. Four document types share one ledger and template family: invoice, retainer, credit note (the only way to reduce an issued invoice - documents are sealed after issue), receipt. `billing_compute()` (pure SQL) is the arithmetic authority; a deferred constraint trigger recomputes every stored figure at commit and refuses disagreement; the shared TypeScript core (`documentMath.ts`, integer cents via BigInt) mirrors it exactly, and the Edge Function cross-checks its own JS total against the database's `billing_compute()` before ever calling the issuing RPC.

**Debt #79 (closed).** `costComponents.ts` + `costReads.ts` (new `_shared` files, database client injected) hold the auction-fee/trucking/freight/duty logic moved out of `bidHeadroomService.ts` (which now re-exports thin wrappers bound to the browser's client). The Edge Function recomputes a `computed` line from the same functions the browser used and refuses the line if the figure disagrees by even a cent - proven live (below). Freight and duty still have no calculator/rate, so a `computed` line naming either is refused outright; those stay `staff_entered` or `document_backed`.

**Builder subagent** (the increment this prompt tested): wrote `documentTemplate.ts` (the PDF renderer) and the logo/font assets in its own files, disjoint from the schema and the Edge Function. Report: three golden PDFs matched the originals with no missing/extra tokens and no position over 4pt off; independently re-run by the main agent with the same result. Assessed in `docs/SOLVED.md` topic 49.

**Live evidence (synthetic orgs only; the real org was never written to except the two confirmed corrections above and the numbering/branding configuration).**
- *INV-0025 reproduced* (ORG 3, external vehicle - `won_vehicle_id` null, `external_plate='DLA49443'`): issued **total ₦2,321,950.00**, every one of 18 lines, the five percent discounts, "Discount Applied -₦148,250.00" (derived, not stored), VAT ₦15,000.00 on a ₦200,000.00 base, the exempt line contributing **₦0.00** - all read back from the stored PDF, matching the original exactly. Printed 2 pages (16+ lines plus the approved per-line origin footnotes push it past 1 page; the footnote block is Bashir-approved, 21 Sep 2026 "keep them as a footnote") - noted as noisy when every line shares one basis, not a defect.
- *INV-0026 reproduced* (ORG 4): total $2,645.00, one $800 deposit applied **at issue**, balance $1,845.00, ₦2,564,550.00 - exact.
- *INV-0027 content reproduced* (ORG 4, same session): total $2,645.00, two deposits ($800+$700) applied at issue, balance $1,145.00, ₦1,591,550.00 - exact. **Honest gap: the document prints as `INV-0028`, not `INV-0027`.** Two of the author's own test-sequencing mistakes (a wrong client id on the first attempt; a synthetic org's counter started one number low) burned two numbers before the correct one issued - voided numbers are never reused, by design, so the label drifted. The content match is exact; the label is not. Recorded here rather than hidden behind a fourth throwaway org.
- *Debt #79 proof, live, through the deployed function:* a tampered $1 auction-fee line - **refused**, with the server's own recomputed figure ($797.50) in the error; the correct $797.50 figure - accepted; an unmatched trucking yard - abstained (no guess), issuance still succeeded once trucking was entered as a staff figure with its basis.
- *Retainer → final invoice → credit note, live:* RET-0001 ($500) issued, paid, credited against INV-0030 ($1,700); a $200 credit note issued against INV-0030. Balances read from `billing_document_balances`: invoice outstanding **$1,000.00** ($1,700 − $500 − $200), retainer outstanding **$0.00**, retainer's own total unchanged at $500.
- *Stage A.6, closed:* `peek_next_document_number` for the real org returns `INV-0028` twice with no allocation (`last_seq` unchanged at 27); a dated-rate grep of every `_shared`/function TS file for a bare `7.5` found none outside comments - every VAT figure traces to `tax_codes`; no "PARTIAL INVOICE"/"NOT the full cost" text anywhere in any live-rendered PDF.
- *Migration 065 (found and fixed live, 22 Sep 2026):* `check_billing_document()` is a shared trigger function fired from both `billing_documents` and `billing_document_lines`, but read `NEW.document_id` even when fired on the documents table (which has no such column) - every first issue attempt failed with `record "new" has no field "document_id"`. Fixed by reading the id through `to_jsonb(NEW)`, keyed on `TG_TABLE_NAME`. One line, no data or rule change; re-verified against all the DB-level test cases before and after.
- *Full DB-level test batch (29 cases, rolled back or reasoned about the live schema, migration 065 applied):* kind rules (retail hides cost lines, no hidden lines outside retail, a scope statement is mandatory), a computed line on an external vehicle/with no source/naming duty all refused, an unknown tax code refused, a negative-rate line refused, deposit double-application refused, an NGN payment converting at the wrong frozen rate refused, a credit note exceeding the balance (alone or cumulatively) refused, crediting an unpaid or already-credited retainer refused, editing or appending to an issued document refused, voiding a document with money applied refused, TRUNCATE refused, a directly-inserted document with a wrong total refused at commit (the tamper-proof test). All PASS.
- Offline: `testDocumentMath.mts` (24/24, incl. the double-subtraction trap on INV-0025's own figures and exact BigInt currency conversion), `testEngineGolden.mts` (rebuilds all three invoices from raw inputs, diffs token-for-token and position-for-position against the original PDFs - PASS), `testCostComponentsParity.mts` (12/12 against the 324 frozen real fee rows, incl. the verified $797.50/$807.50/$802.50/$1,265/$1,280/$1,272.50 figures and every abstention), the frozen Copart regression (7,736 points, 0 differences), `testInvoiceRules.mts` (16/16, legacy Phase 2) and `testPaginatedRead.mts` (10/10). `tsc --noEmit` clean.

**Independent verification (fresh subagent, cold, read-only against the live database except rolled-back transactions).** Full report `docs/verification/prompt38-phaseA.md`. Every row 6-19 PASS, several by the verifier's own adversarial reconstruction rather than trusting the author's tests - it built its own double-subtraction attempt against `billing_compute()` directly and its own tampered-line comparison against `costComponents.ts` (it could not authenticate to the deployed function from its environment; used the documented code-level fallback instead, disclosed as such). No material finding. One real correction came out of it: the verifier could not find a "footer/balance-band overlap" in the original INV-0025 that an earlier pre-flight agent (not the verifier) had reported and that the builder subagent's task description named as a defect to fix - re-checked directly at 150dpi, **no overlap ever existed in the original**. The false claim traced to a pre-flight coordinate misread, was repeated in the builder's brief and its own report, and had reached three code comments but none of the permanent docs; all three corrected to state plainly that nothing was being fixed. Two smaller findings, both low severity: `tax_codes` allows two overlapping *closed* date ranges for one code (no wrong figure is reachable - `billing_compute` refuses to bill against an ambiguous date - only a messy row could persist); the per-org branding proof is real but the two orgs compared happen to hold identical content (the synthetic orgs deliberately mirror Caplimo to reproduce its own invoices).

**Not done / not verified.** Phase B (the client view) pre-flight is running separately (auth/accounts and "what a client sees" done; RLS surface next). A concurrent-write race on the new tables was not re-run live (Phase 2's own race proofs on the untouched old tables still stand; the new guards were proved by single-session tamper attempts, not a live race).
...
### 4.37 Prompt 39 Stages 1-5: the client view, built (22 Sep 2026)

**Status: RLS rewritten and proven (Stage 1); document visibility built and proven (Stage 2); provisioning built, with one real vulnerability found and fixed live (Stage 3); the dashboard built and verified in the browser as a real synthetic client (Stage 4); a hostile-client independent verifier is running (Stage 5, report to follow).** Migrations 066-070 applied; 8 Edge Functions deployed (`client-provisioning` new; `won-vehicle-destination`, `won-vehicle-documents`, `won-vehicle-invoices`, `won-vehicle-notify`, `won-vehicle-winning-bid`, `won-vehicle-billing`, `won-vehicle-status` patched; `billing` extended for client file access).

**Stage 1 (RLS).** See debt #80's closure above. `user_is_staff(org_id)` and `current_client_id(org_id)` are both org-parameterised (a staff member of one org gets no special standing in another). `memberships` gained `revoked_at`/`revoked_by`/`revoke_reason`; `user_org_ids()`/`is_superadmin()` both now exclude revoked rows.

**Stage 2 (document visibility).** `won_vehicle_documents.client_visible` (default false) + `client_visible_by`/`client_visible_at`, set only through a narrow `set_document_client_visibility()` RPC (never a blanket UPDATE grant - the table still has no authenticated UPDATE policy at all beyond this one function). `won_vehicle_documents_visibility_guard` refuses flagging a cost document (`invoice`/`receipt` type) visible when the vehicle has a live retail invoice. Proved live: a brokerage document's flag - accepted; the identical call on a retail vehicle's cost document - refused with the stated reason; default is hidden.

**Stage 3 (provisioning) - the one real vulnerability this prompt exists to catch, found live.** `link_new_auth_user_to_client()` (the existing Google-link trigger) now also creates the client membership it always should have; `provision_client_account()`/`revoke_client_access()` are idempotent and staff-only (proved: two provision calls produce one membership; revoke immediately drops RLS-level access to zero, proved by direct query as that user). A **hostile-client live test** (a real client token, minted directly, never touching the staff session's own storage - see the process note below) found that `won-vehicle-status`'s `advance` mode had **no role check of any kind** - "any authenticated staff member... no role check beyond being a real logged-in user" per its own six-year-old comment, predating the client role entirely. The client token successfully advanced a synthetic vehicle's status before the fix. Fixed (staff/superadmin only, matching every other function), the erroneous history row deleted, redeployed, reconfirmed refused with the same token. All 14 staff Edge Functions then re-swept and confirmed refused for the client token (403 explicit or 404 same-answer masking, the two legitimate refusal shapes already in use elsewhere in the codebase).

A separate, **not fixed**, pre-existing bug found while investigating: `advance_won_vehicle_status()`'s sequence check (`v_new_pos != v_current_pos + 1`) silently passes for a vehicle with NO prior status history at all, because `array_position(sequence, NULL)` is `NULL` and `NULL != anything` is `NULL`, which a PL/pgSQL `IF` treats as false - so the very FIRST status transition on a fresh vehicle can be set to ANY value, bypassing the "must start at won" intent. Recorded as debt #83.

**Stage 4 (the dashboard).** `src/services/clientPortalService.ts` (every read goes through the client's own RLS - no filter of its own) and `src/components/ClientDashboard.tsx`, routed in `App.tsx`'s `AuthGate` on `role === 'client'` - a client never reaches `MainDashboard`, by construction, checked before anything else renders. Reuses the Prompt 38 template only for the actual PDF (signed-URL downloads through `billing`'s extended `file_url` mode and a new `won-vehicle-documents` `client_file_url` mode, both newly scoped to the caller's own `client_id` - no second renderer exists). **Verified live in the browser as the real synthetic client** (not asserted from code): briefs, shared runs, the client's one vehicle with its status ladder, five invoices/retainer/credit-note with correct totals rendered, a retail invoice (INV-0031, $6,200 all-inclusive) expanded to show **exactly one line - the two hidden cost lines never appeared**, a flagged document downloaded successfully, an unflagged/another-client's file refused (404) through the same signed-URL path. Full page text captured and contains nothing beyond this client's own data.

**A process note, disclosed per this prompt's own "output is evidence, not fact" lesson.** Minting the client's test token by writing directly to `localStorage` in one browser tab overwrote the SAME staff session in every tab of this browser (same-origin storage is shared across tabs, which I had wrongly assumed was tab-scoped) - the real staff (Bashir's) browser session was lost mid-verification. Recovered by falling back to direct service-role SQL/Storage calls to finish the one remaining live check (uploading and flagging a real test document) rather than needing the staff UI session back. **Bashir's browser will need a fresh sign-in.**

**Stage 5.** A hostile-client independent verifier attacked every table, Edge Function and storage path directly with the live client token - full report `docs/verification/prompt39-phaseB.md`. **No cross-client or cross-org read/write reached, anywhere**: 18 tables confirmed to return zero rows, direct-id lookups against known real ids in another client's data and the real production org all refused, nine write attempts refused, all 15 staff Edge Functions refused, direct storage access refused. One real finding, same-tenant only: a client querying `clients`/`won_vehicles` directly instead of through the safe views sees extra columns on their OWN row (RLS confines rows, not columns) - `clients.notes`/`assigned_agent` is the one field with genuine sensitive-content risk (was null in the live test); recorded as debt #84, not fixed this session (Postgres cannot column-restrict a role that RLS already row-restricts, since staff and clients share the `authenticated` role - the real fix needs either a table split or a distinct client PostgREST role, both larger than proportionate to add here).

**Not yet done:** the commit is pending confirmation.
### 4.36 Prompt 38 Stage B.1: the client view pre-flight (22 Sep 2026)

**Status: pre-flight complete (three read-only agents, one database-touching, serialised); build not started.** Deliberately: Prompt 38's own stop rule allows "Phase A complete and Phase B not started" as a good outcome, and the pre-flight surfaced a real design question that is Bashir's to answer before any client-facing code is written (below). Rebuilding the document engine (Phase A) was already the bulk of this prompt.

**What exists already.** `org_role_enum` has a `'client'` value, and `supabase/functions/billing/index.ts` already filters `memberships.role !== 'client'` out of staff access - the schema anticipated this phase. `clients.user_id` links to `auth.users` via a Google-OAuth self-link trigger (`link_new_auth_user_to_client()`, migration 026): a client can already sign in and get linked to their own `clients` row by email/phone match.

**What's missing, found by Subagent E.** Nothing inserts a `memberships` row with `role='client'`. A client who links today authenticates successfully, then falls straight into the full staff `MainDashboard` with zero RLS access (no membership → `user_org_ids()` returns nothing) - a broken shell, not a client view. Staff-provisioned invitation (as charter §7 describes) doesn't exist as code anywhere; only the client-initiated link does.

**The real gap, found by Subagent G and confirmed exactly by Subagent F (the RLS-surface agent, the only one with database access).** Every existing staff RLS policy uses one pattern: `org_id IN (SELECT user_org_ids()) OR is_superadmin()` - org-wide, not row-scoped, and `user_org_ids()` is role-agnostic (any membership row, any role, contributes its org). **If a `client` membership existed today with no other change, that client would read every other client's row on every table using this pattern** - confirmed by quoting the exact policy text (not paraphrased) on `clients`, `client_briefs`, `won_vehicles`, `billing_documents`, `billing_payments`, `billing_receipts`, and confirmed textually that the qual contains no predicate that could ever evaluate differently per client. `billing_document_lines_select` has **no reference to `client_visible` at all** - a retail client, under today's policies, could read their own hidden cost lines straight through RLS, not just another client's. `cost_rates` and `auction_fee_brackets` (margin data) are exposed by the identical pattern. None of this is exploitable today (`SELECT role, count(*) FROM memberships GROUP BY role` returns only `superadmin: 2` - no client row exists to test against), but it is a real hole waiting for the first membership row, not a hypothetical one.

**A pre-existing gap this pre-flight re-surfaced, already tracked.** Debt #76 (`research_runs`/`research_run_listings` have a bare `USING (true)` SELECT policy - any authenticated user, any role, already reads every org's research runs) is exactly the shape of hole a client login would walk into first. Not new, but directly relevant now.

**Also found:** `billing_document_balances` computes `outstanding` for voided documents too (no `voided_at IS NULL` filter) - a display concern for a client view, not an RLS concern. RLS is enabled but not forced on every table (`relforcerowsecurity = false`) - a table owner session bypasses it regardless of policy, unchanged from before this phase.

**Proposed fix (not applied - proposal only, from Subagent F, reviewed not yet approved).** Two new role-agnostic-safe helpers (`user_is_staff()`, `current_client_id(org_id)`), then **rewrite** (not add alongside) every staff SELECT policy to `AND user_is_staff()`, then add a client-scoped SELECT arm per table where a client should read anything, plus two column-restricting views (`clients_client_view`, `won_vehicles_client_view` - RLS filters rows, not columns; hiding `clients.notes`/`assigned_agent` and `won_vehicles`' internal linkage needs a view, not a policy). Recommended order: helpers → staff-arm rewrite (zero behavioural change today, closes the door before any client row exists) → client arms. Highest-risk single change: `billing_document_lines`' client arm, the only one needing both a boolean gate (`client_visible`) and a join in the same predicate - get the `AND`/`OR` or the join direction wrong and it silently reopens the exact hidden-cost-line leak this pre-flight exists to close, while looking correct under a staff-only smoke test.

**The open question for Bashir.** `DECISIONS.md` §14.2/14.3 lock `won_vehicle_documents` as "staff-only, behind auth, always" for writes and the public share path, but were written before a client-login RLS design existed and don't explicitly settle a *logged-in client's* read. Subagent F's recommendation is to add no client SELECT arm at all (treat "always" as covering reads too) - stated as a recommendation needing confirmation, not decided here.

**Debts added:** #80 (the RLS gap above), #81 (the won_vehicle_documents client-read question).

**Not started:** provisioning code (nothing calls `auth.admin.inviteUserByEmail` or creates a client membership anywhere), the RLS migrations themselves, any client dashboard UI, Stage B.3 verification, Stage B.4's hostile-client independent verifier.
...
| 36 | No IAAI fee data stored at all (Prompt 21 Phase 1/2) | Every rate in `auction_fee_brackets`/`cost_rates` `auction_fee` is Copart-only. IAAI's own published tables were captured (Standard/High Volume buyer fee, Internet Bid Fee, Service/Environmental/Title-Handling/Premium Imagery fees) but deliberately not stored — zero IAAI invoices exist to cross-check them against, and the Copart cross-check proved a published table can diverge from what a real account actually pays. Store once a real IAAI invoice exists to check against, not before **RECONCILED 21 Sep 2026 (Prompt 36 Stage 3):** the precondition this row set - "store once a real IAAI invoice exists" - is now met: the first real IAAI invoice (the Yaris) arrived 20 Sep and is analysed in §4.29. Still nothing IAAI is stored (`auction_fee_brackets` is 324 rows, all Copart) because the invoice reconciles only partly: the flat fees match, but the buyer-fee schedule is inferred, not read. Status: open, waiting on itemised fee data from Bashir **RECONCILED 21 Sep 2026 (Prompt 37):** the **mechanism** is now closed and proven - an IAAI schedule loads as data (accounts, tiers, brackets, flat fees) with no code change (§4.33). What remains is the DATA only: itemised fee lines from a real IAAI invoice or IAA's own schedule. Waiting on Bashir |
...
| 66 | `trucking_rates.destination_port_normalized` still carries vendor typos (found 20 Sep 2026) | `BATIMORE` (2 rates), `PROVDIENCE` (3), `WILLMINGTON` (1), `MD-BALTIMORE` (1), `GA-RINCON` (1), and `MIAMI` beside `MIAMI PORT` are stored as "normalized" values alongside the correct `BALTIMORE`, `PROVIDENCE`, `WILMINGTON`. A destination saved as a typo variant would match only those few rates. Not corrected: `destination_port_raw` preserves what the vendor sent (§5.8) and fixing the importer's normalisation needs a decision about the canonical set. Mitigation shipped: `destination_options()` reports a rate count so the real ports lead and typo variants are visibly minor | **PARTLY RESOLVED 20 Sep 2026** — see §4.28: the four unambiguous typos (`BATIMORE`, `PROVDIENCE`, `WILLMINGTON`, `MD-BALTIMORE`) are now aliased in the importer's `PORT_ALIASES` and 7 rows corrected. **Still open, deliberately, for Bashir:** `MIAMI PORT` vs `MIAMI` (probably one port; never collide today), `GA-RINCON` (Rincon GA is a distinct inland location, not proven to be Savannah), `DAVISVILLE` (a real port town), and the vendor's regional labels `CALIFORNIA`/`TEXAS`/`NEW JERSEY` **RECONCILED 21 Sep 2026 (Prompt 36 Stage 4):** `MIAMI PORT` -> `MIAMI` merged on Bashir's call (12 Copart RoRo rows; raw text preserved; no duplicate created). `GA-RINCON`, `DAVISVILLE` and the regional labels `CALIFORNIA` / `TEXAS` / `NEW JERSEY` are deliberately left, each with its reason in §4.31 Stage 4. Nothing further is owed on this debt |
| 67 | The yard matcher misses IAAI yards on naming quirks, and the vendor list lacks some IAAI yards (found 20 Sep 2026, after debt #61's relabel) | Of the 62 IAAI-labelled bid.cars lots, 9 remain unmatched: **3 are matcher naming gaps** (`IAA Dallas/Ft Worth (TX)` carries a leading "IAA " the matcher does not strip; `Chicago-North` and `Houston-South` are hyphenated where the vendor list has `Chicago North` and `Houston South`, while the same list writes `Chicago-West` and `Houston-North` hyphenated), **1** is probably an alias (`Minneapolis South` vs the vendor's `Minneapolis/St. Paul`), **3 are yards absent from the vendor sheet** (Honolulu, Santa Clarita, Staten Island), and **1 is not a yard** (`Dream Rides ()`). Not fixed here: the Prompt 20 matcher is deliberately strict (fuzzy matching is how false yard matches get made); any fix should be an explicit reviewed alias/normalisation, not inferred. The Yaris is one of the three naming gaps | **PARTLY RESOLVED 20 Sep 2026:** the hyphen-vs-space cases (`Chicago-North`, `Houston-South`) are fixed by folding a hyphen to a space on both sides in `normalizeCity` (punctuation, within the matcher's own stated rule). Measured on all 202 real sightings: 0 collisions between distinct yards, all 175 existing matches keep the identical yard, exactly 2 new matches (now 177 matched / 25 unmatched). **Still open, deliberately, for Bashir's call:** the leading vendor code in `IAA Dallas/Ft Worth (TX)` (the Yaris; stripping it is a decision about what a name means, and the code leaves the similar `ACE - Perris` prefix unmatched on purpose), the probable alias `Minneapolis South` vs `Minneapolis/St. Paul`, the 3 yards absent from the vendor sheet (Honolulu, Santa Clarita, Staten Island), and the non-yard `Dream Rides ()` | **LEADING "IAA " RESOLVED 20 Sep 2026** (see §4.30): for the IAAI network only, `IAA Dallas/Ft Worth (TX)` now matches the branch `Dallas/Ft Worth`, on the evidence of IAA's own buyer invoice (its Branch column) and the vendor sheet; no IAAI yard begins with "IAA" so nothing can collide; exactly one sighting changes (the Yaris) and all 179 existing matches keep the identical yard. **Still open for Bashir:** `Minneapolis South`, the 3 yards absent from the vendor sheet (Honolulu, Santa Clarita, Staten Island), and the non-yard `Dream Rides ()`; other vendor codes such as `ACE - ` are deliberately not stripped (no evidence yet) **RECONCILED 21 Sep 2026 (Prompt 36 Stage 4):** `Minneapolis South` is recorded as a vendor gap (the sheet has `Minneapolis/St. Paul` and `RICE`; nothing links them), `Dream Rides ()` as a capture-side data-quality issue (not a yard; the matcher abstaining is correct). The remaining item is procurement - Honolulu (2 sightings), Santa Clarita (1), Staten Island (1) and the other absent yards - for Bashir to take to the trucking vendor |
| 68 | Every unpaginated read of `trucking_rates` silently returned only the first 1,000 of 1,740 active rows (found 20 Sep 2026, while making the Yaris's trucking resolve) | PostgREST caps each response at 1,000 rows and a client-side `.limit(2000)` does not lift it. Four call sites loaded the yard list with a bare select - `bidHeadroomService.getInlandTruckingComponent` (every listing's cost panel), `WonVehicleCosts`, `ListingCostBreakdown`, and `searchTruckingYards` - so the yard matcher saw ~57% of the yards (738 of 742 Copart rows, 262 of 588 IAAI, **no Manheim or ADESA**) and a yard in the missing part read as "no yard found", indistinguishable from a real absence. It undermined debt #61/#67 verification (my Node tests used the complete list, the app did not) and - **CORRECTED 21 Sep 2026: this is NOT true of Prompt 20's 65.5% figure**, which (like every figure on record) was measured by script over a complete export and was never affected by the cap; the cap affected only what the *app* did (137/211 = 64.9% matched instead of 176/211 = 83.4%, see §4.31). **RESOLVED 20 Sep 2026:** one paginated, stably-ordered helper `listActiveYardKeys` (plus paging in `searchTruckingYards`), with a completeness check that fails loudly if fewer rows load than the server counts. The app now sees all 531 distinct yards (208 Copart, 187 IAAI, 77 Manheim, 59 ADESA). Only one other table exceeds 1,000 rows, `vehicle_reference_models` (1,135), and every read of it is filtered per make/year. **Watch:** any future unfiltered read of a table that grows past 1,000 rows has the same failure mode **RECONCILED 21 Sep 2026 (Prompt 36 Stage 3):** the sentence "only one other table exceeds 1,000 rows ... every read of it is filtered" is superseded: the Stage 2 audit (§4.31) found `vehicle_reference_models` reads that *could* exceed the cap for a wide make/year range, and paged those and 14 other reads over growable tables (16 in all) |
| 69 | Two live duplicate trucking quotes for the same vendor, yard, port and method (found 21 Sep 2026, Prompt 37) | Copart *International Offsite Auction, MA* -> NEW JERSEY container is quoted at $450 and $525, Copart *Buffalo, NY* -> NEW JERSEY container at $525 and $525 (an exact duplicate), all `Inland Towing`, effective 2025-11-01. The rate lookup takes the cheapest and says "cheapest of N", so nothing is wrong today, but it prevents a live-row unique index on `trucking_rates` (the other two rate tables have one). **Needs Bashir**: which quote is current (close the other), or confirm both are real **Bashir's answer, 21 Sep 2026:** "invoice-128381 is more current and I don't think there are duplicate rates." Checked, and nothing changed: the reference `128381` appears nowhere in the repository or the database (no extraction, won-vehicle document or trucking row), so it cannot be tied to a rate; and the duplicates are real in the live table (Buffalo NY -> NEW JERSEY container: two identical $525 rows; International Offsite Auction MA -> NEW JERSEY container: $450 and $525; all `Inland Towing`, effective 2025-11-01, created 2026-09-07). **Still open**: point me at the vendor-sheet row or upload that invoice and I will close the superseded quote (a dated close, not an edit) - until then no rate was touched **Acted on, 21 Sep 2026 (Bashir: "it's up to you"), the certain half only:** the two **Buffalo NY** rows were a true duplicate (same street, price $525, dates and load timestamp), so one (`8db7e14b-...`) was closed with a dated close (`effective_to = effective_from`, i.e. never live in practice) - logged in `rate_change_log` as `UPDATE/postgres`; the quoted price and the 531 visible yards are unchanged. **The Massachusetts pair is left as it is** ($450 and $525 for *International Offsite Auction* -> NEW JERSEY container, same sheet load): a real conflict I cannot resolve by guessing - either choice moves a price - and the app keeps quoting the cheapest ($450). Still needs the vendor row or that invoice. One duplicate group remains, so the live-row unique index on `trucking_rates` is still deferred |
| 70 | The Gate Fee is labelled "Non-Clean Title" but is added for clean titles too (found Prompt 37) | The flat-fee sum has never filtered by title status, so a clean-title lot carries the $95 gate fee - the documented clean-title figures ($640 at $1,700, $1,130 at $5,000) include it. Preserved unchanged by Phase 1 (existing Copart figures must not move). **Bashir's answer, 21 Sep 2026: not sure.** Left exactly as is. **CLOSED 22 Sep 2026, Prompt 38 Stage 0b: Bashir confirmed Copart does charge the gate fee on clean titles - every figure was correct all along; only the LABEL was wrong.** Superseded `18a66c6f-...` ("Copart Gate Fee (Non-Clean Title)", $95) with a label-only row "Copart Gate Fee", same amount/role/platform/tier, effective 2026-09-22. Fee-row fingerprint (platform, role, applies, value, currency, category, sorted) identical before and after; `always`-fee sum for Copart still $130; the frozen 7,736-point regression still shows 0 differences |
| 71 | `yardMatchingService.ts` hardcodes copart/iaai and has its own "effective platform" resolver, separate from the fee path's data-driven one (found Prompt 37 verification) | Out of scope for the fee requirement but it is the same class of hardcode, and the inland-trucking component (which already holds 588 IAAI and 410 Manheim/ADESA rows) cannot match a new house's yards without a code change. On today's data the two resolvers agree on every sighting. **Claude Code**: read the house list from `auction_houses` there too |
| 72 | The extraction prompt and extraction UI still model bracket `member_account` (found Prompt 37) | `extract-cost-document` still tells the model brackets carry a `member_account`; the review screen now makes the reviewer choose the official fee tier (a foreign key refuses anything else), so nothing wrong can be written, but the model's draft text is stale. **Claude Code**, with a function deploy |
| 73 | Phase 1 cleanup migration 057 (found Prompt 37) | After the code is pushed and deployed: drop `auction_fee_brackets.member_account` and `org_settings.copart_payment_tier`; make `fee_tier` NOT NULL; add the shape CHECK that an `auction_fee` cost rate carries `auction_platform`, `fee_role` and `fee_applies`. Not done earlier because the deployed app still reads/writes those columns. **Claude Code**, after Bashir pushes **DONE 21 Sep 2026:** migration 057 applied after the production bundle was confirmed to contain the new code (`auction_accounts` present, `copart_payment_tier` gone). Dropped `auction_fee_brackets.member_account` and `org_settings.copart_payment_tier`; `fee_tier` is NOT NULL; `cost_rates_auction_fee_shape` CHECK added. The fingerprint of every bracket amount, date, source and tier is identical before and after (`8ef7619b...`, 324 rows); all seven Copart figures still reproduce and IAAI still abstains, checked against the trimmed schema |
| 74 | A paid deposit / commitment fee is not credited against an invoice (found Prompt 37 Phase 2) | The deposit is adopted (`client_briefs.deposit_received_at`, refundable, gates a research run) but no document says how it interacts with an invoice - credit, offset or refund - so no generated invoice does it. **Needs Bashir**: the rule, then a line kind for it |
| 75 | The brokerage fee schedule is still provisional (found Prompt 37 Phase 2) | 7% first $20k / 5% to $60k / 3% above, $500 floor, VAT 7.5% on the fee (`DECISIONS.md` 2.5, "recommended, not yet adopted"). **CLOSED 22 Sep 2026, Prompt 38 Stage 0c: Bashir replaced the provisional percent schedule with a flat policy - a $700 service fee plus VAT at 7.5%, editable per invoice** (discounts are routine). Stored as data (`billing_defaults`, migration 062), not a constant; the dated `VAT` tax code (7.5%, effective 2020-02-01) is separate data the fee references, not embedded in it. The percent brokerage-fee schedule in `DECISIONS.md` 2.5 is superseded, not deleted (history) |
| 76 | `research_runs` and `research_run_listings` have a `USING (true)` SELECT policy for every signed-in user (found Prompt 37 pre-flight) | Migration 002 (`select_research_runs_auth`, `select_research_run_listings_auth`): permissive policies OR together, so any authenticated user can read every org's runs and listings. Harmless with one org and two superadmin users; a real leak with a second org or a client login. Not fixed here (Phase 4, security); the client relationship view does not rely on it (explicit org and client filters). **Claude Code**, Phase 4 |
| 77 | `email_log` has no client link (found Prompt 37 pre-flight) | It joins only through `related_table` / `related_id` (a brief or a won vehicle), so a client's email history is a two-hop query, and 8 briefs have `confirmation_sent_at` with no log row (sent before migration 044). The relationship view does the two-hop join; a `client_id` column would make it one. **Claude Code**, low priority |
| 78 | `currencyService.fetchExchangeRates` silently falls back to a hard-coded NGN rate (found Prompt 37 Phase 2) | On any failure it returns fallbacks (NGN 1500) and `convertToUSD` returns the unconverted amount if a rate is missing. The invoice path deliberately does not use it (it fetches its own rate and refuses on failure), but the Document Extraction path (`resolveCurrencyFields`) can still freeze a fallback rate as if it were fetched. **Claude Code** |
| 79 | Computed invoice lines (auction fees, trucking, freight) are trusted from the browser, not recomputed on the server (found by the Prompt 37 Phase 2 verifier) | The cost services live in frontend code (`bidHeadroomService`), so `won-vehicle-billing` can verify only the vehicle price (against the winning-bid ledger) and that a computed line carries a source; a session holder could label any figure `computed`. **CLOSED 22 Sep 2026, Prompt 38 Phase A.** The auction-fee/trucking/freight/duty cores moved to `supabase/functions/_shared/costComponents.ts` + `costReads.ts` (database client injected); `bidHeadroomService.ts` now re-exports thin wrappers bound to the browser's client. The `billing` Edge Function recomputes a `computed` line from the same functions and **refuses it if the figure disagrees by even a cent** - proved live: a tampered $1 auction fee refused with the server's own $797.50 in the error; the correct figure accepted. Freight and duty still have no calculator/rate, so a `computed` line naming either is refused outright (must be `staff_entered` or `document_backed`) |
| 80 | RLS policies are role-agnostic and org-wide, not row-scoped - a client-role membership (none exist yet) would read every other client's rows, including hidden retail cost lines via `billing_document_lines` (no `client_visible` filter in RLS at all), and margin data via `cost_rates`/`auction_fee_brackets` (found by Prompt 38 Stage B.1's RLS pre-flight) | **CLOSED 22 Sep 2026, Prompt 39 Stage 1.** Migrations 066-068: `user_is_staff(org_id)`/`current_client_id(org_id)` helpers; every one of 59 policies matching the canonical org-wide pattern rewritten by a generic sweep (not a hand-picked list); two column-restricted views (`my_client_record`, `my_won_vehicles`); direct Storage bucket access moved to staff-only. Proved live (rolled-back transactions): two real client identities in the designated test org, zero cross-reads in either direction across 12 tables (direct-id lookups, not just list absence), all writes refused, staff reads byte-identical before/after (3/12/2/7/2). Also closed two leaks beyond what the pre-flight named: `assets` and `sightings` each had the same `SELECT USING (true)` hole debt #76 only recorded for `research_runs`/`research_run_listings` |
| 81 | Whether a logged-in client should ever read `won_vehicle_documents` is unsettled (found by Prompt 38 Stage B.1) | **CLOSED 22 Sep 2026, Prompt 39 (Bashir's decision, recorded in the prompt itself).** Clients read their own documents, gated by a new `client_visible` flag (default false, staff-set, migration 069) - superseding the "staff-only always" reading of `DECISIONS.md` 14.2/14.3 (dated supersession note, `DECISIONS.md` 20.1). A cost document (`invoice`/`receipt` type) cannot be flagged visible on a vehicle invoiced retail - enforced by a trigger, not the UI; proved live (flag accepted on a brokerage document, refused with the exact reason on a retail one) |
| 82 | `won-vehicle-status`'s `advance` mode had NO role check at all - any authenticated user, including a client token, could change a won vehicle's status (found LIVE by Prompt 39's own hostile-client test, before Stage 5's independent verifier even ran) | The function's own six-year-old comment said "any authenticated staff member - no role check beyond being a real logged-in user", predating the client role entirely. A live client token successfully advanced a synthetic vehicle's status. **CLOSED 22 Sep 2026**: staff/superadmin check added, matching every other function; the erroneous history row deleted; redeployed; reconfirmed refused with the same token |
| 83 | `advance_won_vehicle_status()`'s sequence check silently passes for a vehicle with no prior status history, letting the FIRST transition be set to any status (found while investigating #82) | `array_position(sequence, NULL)` is NULL, and PL/pgSQL treats a NULL `IF` condition as false, so the "must advance by exactly one step" check never fires when `v_current` is NULL. Not a client-isolation issue (staff could always do this); a data-integrity gap in the status ladder itself. Fix: treat NULL current status as position 0, requiring the first real transition to be `won` (position 1). **OPEN** |
| 84 | A client can read extra columns on their OWN `clients`/`won_vehicles` rows by querying the base table directly instead of the safe `my_client_record`/`my_won_vehicles` views - RLS confines rows, not columns (found LIVE by Prompt 39 Stage 5's hostile-client verifier) | Same-tenant only, never another client's row - confirmed exhaustively by the same verifier (18 tables checked, direct-id lookups against known real ids in another client and the real org, all refused). `won_vehicles`' extra columns are opaque internal ids with no exploitable content (`share_token` isn't secret from its own client - staff hand it to them directly). `clients.notes`/`assigned_agent` is the one real risk (staff-private commentary about the client); was `null` in the live test. Postgres cannot column-restrict a role RLS already row-restricts - staff and clients share `authenticated`. Real fix: move `notes`/`assigned_agent` to a staff-only table (~9 call sites in `researchService.ts` + `ClientsList.tsx`), or give client sessions a distinct PostgREST role. **OPEN** |
