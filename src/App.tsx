...
// --- CUSTOM HOOK FOR FILE STORAGE ---
function useFileStorage(data: any) {
  const [fileHandle, setFileHandle] = useState<FileSystemFileHandle | null>(null);

  const openFile = async () => {
    try {
      // @ts-ignore - File System Access API types might be missing in some setups
  ...
  const saveAsNewFile = async () => {
    try {
      ...
      const handle = await window.showSaveFilePicker({
        suggestedName: 'autodata_db.json',
        types: [{ description: 'JSON Database', accept: { 'application/json': ['.json'] } }],
      });
      setFileHandle(handle);
    } catch (err) {
  ...
  return { openFile, saveAsNewFile, isLinked: !!fileHandle };
}
...
const App: React.FC = () => {
  ...
  // File Storage Hook
  const { openFile, saveAsNewFile, isLinked } = useFileStorage(sales);
  ...
  useEffect(() => {
    ...
    // 2. Load Sales from LocalStorage (Initial Load)
  ...
  const handleLoadFromFile = async () => {
    const loadedData = await openFile();
    if (loadedData && Array.isArray(loadedData)) {
      setSales(loadedData);
      importSales(loadedData); // Also sync to browser local storage for backup
  ...
  const stats: CarStats = useMemo(() => {
    ...
    return { totalSold: sales.length, totalVolume: totalVolumeDisplay, avgDaysToSell: countDays > 0 ? totalDays / countDays : 0, fastestMoving: fastest, topModels, topDealers };
    ...
    <div className="min-h-screen bg-[#F0EDDE] text-[#403f4c] font-sans relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center bg-[#F0EDDE] rounded-lg p-1 mr-2">
                 <button 
                    onClick={handleLoadFromFile}
                    className="p-1.5 text-[#403f4c]/70 hover:text-[#a58039] hover:bg-white rounded-md transition-all shadow-sm flex items-center gap-1"
                 ...
                 {isLinked ? (
                    <span className="text-xs font-bold text-green-600 px-2 flex items-center">
              ...
              <div className="flex items-center bg-[#F0EDDE] rounded-lg p-1">
                  ...
                  <div className="w-px h-5 bg-[#a58039]/20 mx-1"></div>
                  <button onClick={() => setShowExportModal(true)} className="p-1.5 text-[#403f4c]/70 hover:text-[#a58039] hover:bg-white rounded-md transition-all shadow-sm" title="Export to Excel/CSV">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {view === 'dashboard' ? (
          ...
          <CarTable sales={sales} onDelete={handleDeleteSale} onBulkDelete={handleBulkDelete} onEdit={handleEditSale} displayCurrency={displayCurrency} exchangeRates={exchangeRates} />
        )}
      </main>
    </div>
  );
